...
String toString() {
    ...
    return ret.substr(
        beginPos + 1, ret.size() - beginPos - static_cast<String::size_type>(sizeof(">(void)"))
    );
#else
...
struct filldata<
    ...
    typename detail::types::enable_if<
        !detail::has_insertion_operator<T>::value && detail::is_pair<T>::value>::type> {
    static void fill(std::ostream *stream, const T &in) {
...
struct DOCTEST_INTERFACE ExpressionDecomposer {
    ...
    template <
        typename L,
        typename types::enable_if<!doctest::detail::types::is_rvalue_reference<L>::value, void>::
            type * = nullptr>
    Expression_lhs<const L &> operator<<(const L &operand) {
...
#define DOCTEST_INTERNAL_ERROR(msg)                                              \
    detail::throw_exception(std::logic_error(__FILE__ ":" DOCTEST_TOSTR(__LINE__ \
    ) ": Internal doctest error: " msg))
...
const char *assertString(assertType::Enum at) {
    ...
    )  // enum 'x' in switch of enum 'y' is not explicitly handled
...
bool matchesAny(
    const char *name, const std::vector<String> &filters, bool matchEmpty, bool caseSensitive
);
