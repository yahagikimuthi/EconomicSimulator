...
def generateKdeGif(df: pd.DataFrame, columnName: str, filename: str, titleTemplate: str ="Distribution Dynamics [{frame}]", 
                   xlabel: str ="Value", color: str ="teal", stepInterval: int =2) -> None:
    ...
    ani: Final[animation.FuncAnimation] = animation.FuncAnimation(
        fig, update, frames=range(0, maxStep + 1, stepInterval), blit=True
    )
